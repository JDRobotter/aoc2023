...
impl Schematic {
    ...
    fn is_possible_gear(&self, c: char) -> bool {
        c == '*'
    }
    ...
    fn extract_part_number(&mut self, x: isize, y: isize) -> Option<usize> {
    ...
    pub fn check_gear(&mut self, x: isize, y: isize) {
        let positions = [
            (-1, -1),
            (-1, 0),
            (-1, 1),
        ...
        for (dx, dy) in positions {
            ...
            if let Some(c) = self.get(x, y) {
                if c.is_numeric() {
                    // we found a part number
                    if let Some(pn) = self.extract_part_number(x, y) {
                        pns.push(pn);
                    }
                }
            }
